 ...
 *
 * PASSE DU 2 SEPTEMBRE 2026 — v3. Identique a v2 sauf trois choses :
 *
 *   1. La zone est le DEPARTEMENT Maine-et-Loire, une seule cible geographique,
 *      au lieu des dix communes. Les dix communes etaient une contrainte d'outil
 *      (generateKeywordIdeas plafonne a 10 geo_target_constants), pas un choix de
 *      marche : la zone declaree par le client est "Brissac, Angers et
 *      Maine-et-Loire, rayon 30 km". Le departement etant UNE cible, la limite
 *      disparait et la decouverte porte enfin sur toute la zone.
 *   2. La fenetre de prevision est 03/09 -> 02/10/2026. Celle de v2 (le mois de
 *      septembre) commence dans le passe et serait refusee.
 *   3. Les GOOGLE_ADS_* sont absents du .env.local de ce checkout : ils viennent
 *      du vault Obsidian.
 *
 * Rien ne se compare terme a terme avec la passe du 04/08 : la zone ET la fenetre
 * ont change, et Google reprevoit en continu (mesure a +5 % en un jour le 05/08).
 *
 *   node --import tsx scripts/audit-gp-elec-v3-departement.mjs
 */
...
const OUT = `${AUDIT}/data/donnees-google-ads-brutes-2026-09-02.json`;
...
// La fenetre de v2 commence dans le passe : Google la refuserait. 30 jours pleins.
const PERIOD = { start_date: "2026-09-03", end_date: "2026-10-02" };
const SITE = "https://gp-elec-49.com";
// UNE seule cible geographique : le departement. C'est le changement de v3.
const COMMUNES = ["Maine-et-Loire"];
...
const out = {
  meta: {
    ...
    url_seed: SITE,
    zone: COMMUNES,
    zone_note: "Departement Maine-et-Loire, une cible geographique. La passe du 04/08 portait sur dix communes, par contrainte d'outil.",
    budgets: BUDGETS,
...
const GEO = out.geo.map((g) => g.geo);
  console.log(`  ${GEO.length}/${COMMUNES.length} cible(s) geo resolue(s) : ${GEO.join(", ")}`);
save();
